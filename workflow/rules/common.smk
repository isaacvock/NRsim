...
def get_target_output():
    ...
    if config["dataset_specific"]:
        ...
        if PE:
            ...
            target.append(expand("results/shuffle_fastq/{sim}/sample_{sample}/read_{read}.fastq.gz", sim = SIM_NAMES, sample = sample_names, read = READS))

        else:

            target.append(expand("results/convert_to_fastq/{sim}/sample_{sample}.fastq.gz", sim = SIM_NAMES, sample = sample_names))
            target.append(expand("results/shuffle_fastq/{sim}/sample_{sample}/read_1.fastq.gz", sim = SIM_NAMES, sample = sample_names))
        ...
        if PE:
            ...
            target.append(expand("results/shuffle_fastq/sample_{sample}/read_{read}.fastq.gz", sample = sample_names, read = READS))

        else:

            target.append(expand("results/convert_to_fastq/sample_{sample}.fastq.gz", sample = sample_names))
            target.append(expand("results/shuffle_fastq/sample_{sample}/read_1.fastq.gz", sample = sample_names))
